...
export default function Intro({ detailss, visitor, setOthername }) {
  ...
  const [max, setMax] = useState(infos?.bio ? 250 - infos?.bio.length : 250);

  const updateDetails = async () => {
    try {
      const { data } = await axios.put(
        `${serverConfig.port.backendUrl}/${serverConfig.apiPath.users}/update_details`,
        {
          infos,
        },
        {
  ...
  return (
    <div className="profile_card">
      ...
      {details?.otherName && (
        ...
        <img src="../../../icons/alias.png" alt="" />
          Also Known as {details?.otherName}
        </div>
      )}
